//contexto thisBinding
/**
 * Sempre que executamos uma função no JavaScript, ela é associada a um contexto de execução.
 *  Esse contexto possui uma propriedade denominada ThisBinding, que pode ser acessada a qualquer momento através 
 * da palavra reservada this. O valor do this, que chamamos de contexto da função, 
 * é constante e existe enquanto este contexto de execução existir. 
 *As arrow functions foram projetadas para conseguirmos capturar o this (ou seja, o contexto de execução) do 
 seu contexto delimitador (chamamos isso de escopo léxico da função). 
 Com isso, não precisamos mais utilizar métodos com o bind ou fazer truques como armazenar o valor do contexto em uma variável.
 */

const equipe = {
    nome: 'Linguagem de programação',
    membros: ['JavaScript','Java','Python', 'Ruby'],
    membrosDaEquipe: function() {
        let that = this;
        this.membros.forEach( function(membro) {
            console.log(`${membro} pertence a equipe ${that.nome}`);
        } )
    }
}
...
const equipe2 = {
    ...
    membrosDaEquipe: function() {
        this.membros.forEach( (membro) => {
            console.log(`${membro} pertence a equipe ${this.nome}`);
        } )
    }
}
